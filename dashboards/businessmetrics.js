// dashboards/businessmetrics.js
// Iteration 4: Business Metrics
// Step 1 = dropdown + KPI numbers
// Step 2 = daily activity line chart (views / saves / booking clicks)
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Export report as it opens print dialog, user can "Save as PDF"
    ...
    <div class="quick-pick-list">
      ${picks.map((e) => `
        <button type="button" class="quick-pick" data-exp="${e.experience_id}">
          ...
          <span class="quick-pick-sub muted">Last 7 days</span>
        </button>
      `).join('')}
    </div>
  `;
...
function bindExportButton() {
  document.getElementById('btnExport')?.addEventListener('click', () => {
    alert('Export coming next step (Iteration 4 enhancement).');
  });
}
...
/**
 * Adds/removes the "is-active" class on the metric chips.
 */
...
/**
 * Loads daily totals from your views and builds 3 arrays:
 * - views per day
 * - saves per day
 * - booking clicks per day
 *
 * Then it calls renderActivityChart() with that data.
 */
async function refreshDailyActivity() {
  if (!currentBusiness) return;

  // Your views use DATE, so we filter using YYYY-MM-DD
  const startDateStr = sinceISO(currentDays).slice(0, 10);
  const endDateStr = new Date().toISOString().slice(0, 10);

  // Build the full list of days so missing days still show as zero
  const days = buildDayList(startDateStr, endDateStr);

  // Pull both datasets at the same time
  ...
  // Convert rows into maps (day -> total)
  ...
  // Build arrays aligned with "days"
  const series = {
    days,
...
async function refreshTopReferrers() {
  ...
  // Pull only what we need and count sources in JS (simple + student friendly)
  let q = supabaseClient
    .from('event_metric_v')
  ...
  // Optional: tie referrers to views only (makes more sense as "traffic source")
  ...
  // Sort biggest -> smallest, take top 6, group rest as "other"
  ...
  // Palette: primary green + neutrals
  const colors = [
    '#16a34a', // green
    '#111827', // black
    '#6b7280', // grey
 ...
 * If the user selects "Views", we show one line.
 * If they select "All", we show all 3 lines.
 */
function renderActivityChart(series) {
  ...
  // Decide what lines to show (and force ExperienceMe colours)
const GREEN = '#16a34a';
const BLACK = '#111827';
const GREY  = '#9ca3af';
...
function renderFunnel({ views, saves, clicks }) {
  ...
  box.innerHTML = `
    ${funnelRow('Views → Saves', clampPercent(viewsToSaves))}
    ${funnelRow('Saves → Booking clicks', clampPercent(savesToClicks))}
    ${funnelRow('Views → Booking clicks', clampPercent(viewsToClicks))}
  `;
}
